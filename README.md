...
```mermaid
graph TD
    A[User via Client] -->|HTTP POST Request| B(FastAPI: /recommend_crops);
    B -->|temperature, humidity, etc.| C{recommender.py};
    C -->|Hybrid Search Query| D[Astra DB Vector Store];
    D -->|Top 4 Crop Results| C;
    C -->|Create Prompt| E[Groq LLM API];
    E -->|Generated Explanation| C;
    C -->|JSON Response| B;
    B -->|Crops + Explanation| A;
```
...
ASTRA_DB_CROP_API="AstraCS:..."
ASTRA_DB_CROP_ID="your_database_id"
```
...
![image](https://github.com/user-attachments/assets/796e5905-7fea-4cd2-865e-b3167bc19c7b)
